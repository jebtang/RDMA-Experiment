...
if [ "X$distro" == "X" -a $deb -eq 0 ]; then
	...
	case $distro_rpm in
		...
		distro=rhel7.3
		;;
		redhat-release*-7.4*el7a*|centos-release-7-4*el7a*|sl-release-7.4*el7a*)
...
if [ $skip_repo -eq 0 ]; then
	...
	ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $TMP --mofed $tmpdir/$iso_dir --rpms-tdir $tmpdir/$iso_dir/$BINDIR --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM $METAGROUPS
